...
export default function ShirtModel() {
  ...
  // Keep targetMeshRef around to satisfy types or legacy refs if needed, using the largest mesh
  ...
  const activeConfig: any = (() => {
    ...
    return {
      ...
      decalDepth: THREE.MathUtils.clamp(size.z * 0.2, 0.05, 0.2), // Shallow enough not to hit the back
      // Map the 500x600 2D canvas to completely cover the height and width of the 3D model, expanded slightly to reach the hood
      printW: size.y * (500 / 600) * 1.15, 
      printH: size.y * 1.15,
      useUniformScale: true,
      centerX: center.x,
      centerY: center.y + size.y * 0.1, // Shift the projection area up 10% to comfortably reach the hood and neck
    };
  })();
