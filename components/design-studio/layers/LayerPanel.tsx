...
import { Type, Image as ImageIcon, Minus, Eye, EyeOff, Lock, Unlock, Copy, Trash2, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function LayerPanel() {
  const { 
    ...
    selectLayer, removeLayer, duplicateLayer, 
    reorderLayer, toggleLayerVisibility, toggleLayerLock,
    renameLayer
  } = useDesignStore();
        ...
        {sortedLayers.map(layer => {
          ...
          return (
            ...
            >
              ...
              <div className={styles.layerName} onDoubleClick={() => setEditingLayerId(layer.id)}>
                {editingLayerId === layer.id ? (
                  <input
                    ...
                    onBlur={(e) => {
                      renameLayer(layer.id, e.target.value || layer.type);
                      setEditingLayerId(null);
                    }}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        renameLayer(layer.id, e.currentTarget.value || layer.type);
                        setEditingLayerId(null);
                      }
                    }}
                  />
              ...
              <div className={styles.actions}>
                ...
                >
                  {layer.isVisible ? <Eye size={14} /> : <EyeOff size={14} style={{ color: '#888' }} />}
                </button>
                <button 
                  className={styles.actionBtn} 
                ...
                >
                  {layer.isLocked ? <Lock size={14} style={{ color: '#888' }} /> : <Unlock size={14} />}
                </button>
