...
import styles from "./onamcollections.module.css";
        ...
        const fetchOnamCollections = async () => {
            try {
                const res = await getProducts({
                    limit: 20, // 🔥 IMPORTANT
                    tags: ["ONAM COLLECTION"],
                });
                console.log("Products Response:", res);

                setProducts(res.items || []);
            } catch (err) {
                console.error("Failed to fetch onam collections", err);
            }
        };

        fetchOnamCollections();
    }, []);
    ...
    return (
        <section className={styles.section}>
            <div className={styles.header}>
                <h2 className={styles.title}>ONAM COLLECTIONS</h2>
            ...
            <div className={styles.viewAllWrapper}>
                <button
                    ...
                    onClick={() => router.push("/products?tag=ONAM%20COLLECTION")}
                >
                    VIEW ALL PRODUCTS
                </button>
            </div>
...
export default OnamCollections;
